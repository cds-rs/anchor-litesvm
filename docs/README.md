...
| **Why is it built this way?** (architecture, internals, tradeoffs) | [`adr/`](adr/) for the design premise and the settled decisions (start at [ADR 0000](adr/0000-the-premise-trait-boundaries-and-ecosystem-reach.md)); [`design/`](design/): [trait-boundaries.md](design/trait-boundaries.md) for the layered crates and the `TestSVM` port, [cpi-rendering.md](design/cpi-rendering.md) for the renderer family, [bundled-pubkeys.md](design/bundled-pubkeys.md) for the derive, [litesvm-boundary.md](design/litesvm-boundary.md) for the litesvm/anchor-litesvm split. | Committed markdown, subject to review. The internals they describe are `pub(super)`, so they do not appear in rustdoc; the design docs are their home. |
| **How do I move an existing raw-LiteSVM suite over?** | **The book**: [Migrating from Raw LiteSVM](../book/src/appendix/migration.md). | Hand-written, subject to review. |
| **What have we offered back upstream?** | [`UPSTREAMING.md`](UPSTREAMING.md): the boundaries that graduated from our fork into PRs against agave, litesvm, and surfpool (the visible output of [ADR 0000](adr/0000-the-premise-trait-boundaries-and-ecosystem-reach.md)). | Each entry carries a live PR/issue link and a status, kept current by review. |
