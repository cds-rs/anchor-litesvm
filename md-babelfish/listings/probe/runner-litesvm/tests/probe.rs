//! The probe spec on litesvm. Same spec, different engine: only the two lines
//! that name the engine change between runners.

// ANCHOR: runner
use {litesvm::LiteSVM, litesvm_utils::LiteSvmBackend, testsvm::TestSVM};
...
#[test]
fn counter_on_litesvm() {
    let mut engine = LiteSvmBackend::new(LiteSVM::new());
