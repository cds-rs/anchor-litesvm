...
/// The hook [`Tx::build`] runs on a bundle before projecting it onto
/// account metas: resolve anything that must be computed against live SVM
/// state. Bundles generated by [`bundles_from_idl!`](crate::bundles_from_idl)
/// derive their PDAs eagerly in `From` and implement this as a no-op; a
/// hand-written bundle whose fields depend on on-chain state (a
/// counter-seeded PDA, say) computes them here.
pub trait Resolvable {
    fn resolve_all(&mut self, ctx: &crate::AnchorContext);
}
...
impl<'a> Tx<'a> {
    ...
    {
        // The pre-projection hook: late-bound fields resolve against live
        // SVM state here; eagerly-derived bundles make this a no-op.
...
mod tests {
    ...
    // A hand-rolled bundle implements the build-time resolve hook itself:
    // nothing late-bound, nothing to resolve.
