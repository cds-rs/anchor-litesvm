...
//! ctx.send_ok(ix, &[&signer]).print_logs_structured();
//! ```
//!
//! and the failure-path twin is *another* method per verb on top, so
//! test-helper modules end up doubling every verb (the 02-amm dogfood
//! had 21 of these pairs in `common/mod.rs`). With `Tx`, the chain is
//! one statement and the happy/failure paths share everything up to
//! the terminator:
//!
//! ```ignore
//! ctx.tx(&[&signer])
...
impl<'a> Tx<'a> {
    ...
    /// variable-length account set the fixed-account [`Bundle`](crate::Bundle)
    /// cannot model. The fixed accounts get their names from the bundle; the
    /// tail stays positional, and the rendered authority / tree views name it
    /// through aliases anyway. Requires a prior `.build()` / `.build_with()` /
    /// `.ix()` call.
    ///
    /// ```ignore
    /// ctx.tx(&[&session])
