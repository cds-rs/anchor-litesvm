...
// The RPC execution backend (a `TestSVM` that talks to a surfnet/cluster) is
// gated behind the same `rpc` feature that pulls in its RPC-client deps.
#[cfg(feature = "rpc")]
pub use litesvm_utils::RpcBackend;
...
mod integration_tests {
    ...
    /// `print_logs_structured()` / `logs_structured_string()` resolves them.
    #[test]
    fn anchor_context_send_ok_threads_self_aliases_through_to_result() {
        ...
        ctx.alias(payer.pubkey(), "payer-from-ctx");

        let ix = system_instruction::transfer(&payer.pubkey(), &recipient.pubkey(), 1_000_000);
        let out = ctx.send_ok(ix, &[&payer]).logs_structured_string();

        assert!(
            out.contains("signer=payer-from-ctx"),
            "ctx.alias should flow through ctx.send_ok to no-arg render; got:\n{out}"
        );
        assert!(
            out.contains("  payer-from-ctx = "),
            "ctx-owned alias should surface in legend; got:\n{out}"
        );
    }
    ...
    #[test]
    fn anchor_context_is_a_testsvm_engine() {
        use litesvm_utils::TestSVM;

        let mut ctx = AnchorContext::new(LiteSVM::new(), Pubkey::new_unique());

        // Trait vocabulary works on AnchorContext directly: `actor` is inherited
        // as a default method, `label` resolves the alias it registered.
        let alice = ctx.actor("Alice", 1_000_000_000);
        assert_eq!(ctx.label(&alice.pubkey()), "Alice");

        // `get_account` now means the trait's raw accessor (the deserializing
        // inherent method became `try_load`), so this returns Option<Account>.
        let raw: Option<solana_account::Account> = ctx.get_account(&alice.pubkey());
        assert!(raw.is_some());

        // And AnchorContext satisfies `impl TestSVM`.
        fn label_via_trait(svm: &impl TestSVM, pk: &Pubkey) -> String {
            svm.label(pk)
        }
        assert_eq!(label_via_trait(&ctx, &alice.pubkey()), "Alice");
    }
