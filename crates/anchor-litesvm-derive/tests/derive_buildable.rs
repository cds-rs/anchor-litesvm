...
pub mod instruction {
    ...
    // Hand-rolled AnchorSerialize so the test compiles without a
    // direct `borsh = "0.10"` dep (anchor 0.31's derive expands to
    // `::borsh::maybestd::*`, which the test crate's namespace can't
    // resolve when the workspace borsh is 1.5).
    pub struct Make {
        pub amount: u64,
    }
