...
enum FieldSource {
    ...
    /// etc.). Boxed: `Expr` runs ~240 bytes vs. `Project`'s `Ident` at ~32.
    Expr(Box<Expr>),
}
...
fn extract_fields(input: &DeriveInput, fixtures: &[(syn::Ident, syn::Type)]) -> Result<Vec<Field>> {
...
fn extract_from_attr(field: &syn::Field) -> Result<Option<Box<Expr>>> {
    let mut found: Option<Box<Expr>> = None;
