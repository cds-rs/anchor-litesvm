...
impl<'a> LegendCollector<'a> {
    ...
    /// Resolve a decoded instruction's typed operands to display strings through
    /// the alias table (the render-half of the [interpretation
    /// layer](crate::interpret)). A `Pubkey` operand becomes its alias (or short
    /// key), `Lamports` its comma-grouped digits. Does not record into the legend
    /// (the parties surface as accounts elsewhere in the render).
    pub(super) fn resolve_operands(
        &self,
        operands: &[(String, crate::interpret::Operand)],
    ) -> Vec<(String, String)> {
        crate::interpret::resolve_operands(operands, self.aliases)
    }
