...
fn render_frame(
    ...
    // The instruction label: the name plus the decoded operand summary (a
    // `System::Transfer`'s `(from -> to) N lamports`), aliased through the
    // collector. Text uses the ASCII arrow.
    ...
    let label = match (&resolved.name, summary.is_empty()) {
        (Some(name), false) => Some(format!("{name} {summary}")),
        (Some(name), true) => Some(name.clone()),
        (None, false) => Some(summary),
        (None, true) => None,
    };
    let _ = match &label {
        Some(label) => write!(
            out,
            "{}{}{}::{} [{}]",
            ancestor_prefix, connector, program_display, label, depth
        ),
        None => write!(
            out,
            "{}{}{} [{}]",
...
mod tests {
    ...
    #[test]
    fn renders_transfer_operands_aliased() {
        // A decoded `System::Transfer` carries typed operands; the rich tree
        // resolves them through the alias table so the value reads in role terms
        // and no raw key leaks (parity with `pretty_cpi_tree`).
        ...
        let model = crate::cpi::model::CpiModel {
            header: None,
            roots: vec![crate::cpi::model::Root { signers: vec![], frame }],
            tx_signers: vec![],
            error: None,
            compute_units: 0,
            fee: 0,
