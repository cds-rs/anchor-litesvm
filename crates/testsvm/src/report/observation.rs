...
/// A decoded operand, projected for serialization: a `Pubkey` as its base58
/// string (role-mapped later, by normalize, like `program`), a `Lamports` value
/// kept numeric. The serialization mirror of [`crate::interpret::Operand`], so
/// the record stays plain strings/numbers and needs no pubkey serde feature.
...
impl FactOperand {
    fn from_operand(operand: &crate::interpret::Operand) -> FactOperand {
        match operand {
            crate::interpret::Operand::Pubkey(pk) => FactOperand::Pubkey(pk.to_string()),
            crate::interpret::Operand::Lamports(l) => FactOperand::Lamports(*l),
        }
    }
}
...
/// table), the dispatched instruction name, the decoded operands, the outcome as
/// a stable tag, the consumed CU (kept here, dropped during normalize), and the
/// CPI children.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FactFrame {
    ...
    #[serde(default)]
    pub operands: Vec<(String, FactOperand)>,
    pub outcome: String,
    ...
    fn from_frame(frame: &crate::frame::Frame) -> FactFrame {
        ...
        FactFrame {
            ...
            operands: frame
                .operands
                .iter()
                .map(|(k, v)| (k.clone(), FactOperand::from_operand(v)))
                .collect(),
            outcome,
