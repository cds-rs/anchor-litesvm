...
impl Transaction {
    ...
    pub fn assemble(
        ...
        // Native builtins (System, ...) log no `Instruction:` line, so a CPI into
        // them lands here unnamed; decode the name from the trace data. Runs for
        // every engine (frames built from logs or from the trace alike), so the
        // render stays identical across engines. Only fills `None`, so a logged
        // name still wins.
        if let Some(trace) = &trace {
            crate::frame::name_native_frames(&mut frames, trace);
        }
    ...
    /// Whether the transaction succeeded: no transaction-level error message.
    pub fn is_success(&self) -> bool {
        self.error.is_none()
    }

    /// The custom program error code, if the transaction failed with one.
    ///
    /// Read from the raw logs, not the top-level `error` field: `error` is each
    /// engine's native error type stringified (so its shape is engine-specific),
    /// whereas `custom program error: 0x<code>` is the agave runtime's own log
    /// line, identical on litesvm, mollusk, and quasar. Returns the first such
    /// code in log order (the failing frame's). An engine whose failures don't
    /// take that shape resolves them through its [`FailureResolver`] instead.
    ...
    /// Assert the transaction succeeded; returns it for chaining. Panics with
    /// the failure message and the logs otherwise.
    pub fn assert_success(self) -> Self {
        assert!(
            self.is_success(),
            "expected the transaction to succeed, but it failed: {}\nlogs:\n{}",
            self.error.as_deref().unwrap_or("<no message>"),
            self.logs.join("\n"),
        );
        self
    }

    /// Assert the transaction failed with custom program error `code`; returns
    /// it for chaining. Matches the runtime's `custom program error: 0x<code>`
    /// (see [`error_code`](Self::error_code)), so it holds across engines.
    pub fn assert_error_code(self, code: u32) -> Self {
        match self.error_code() {
            Some(actual) if actual == code => self,
            actual => panic!(
                "expected custom program error 0x{code:x}, got {}\nlogs:\n{}",
                actual.map_or_else(|| "no custom error".to_string(), |a| format!("0x{a:x}")),
                self.logs.join("\n"),
            ),
        }
    }
...
mod tests {
    ...
    fn tx_with(logs: Vec<String>, error: Option<String>) -> Transaction {
        Transaction {
            frames: vec![],
            account_keys: vec![],
            logs,
    ...
    #[test]
    fn error_code_reads_the_runtime_custom_error_from_logs() {
        // The runtime's `custom program error: 0x<code>` is the engine-invariant
        // source; `error_code` parses the hex from it regardless of the
        // engine-specific top-level `error` string.
        let tx = tx_with(
            vec![
                "Program Vote111 invoke [1]".to_string(),
                "Program Vote111 failed: custom program error: 0x7b".to_string(),
            ],
            Some("InstructionError(0, Custom(123))".to_string()),
        );
        assert!(!tx.is_success());
        assert_eq!(tx.error_code(), Some(123));
        tx.assert_error_code(123);
    }

    #[test]
    fn assert_success_holds_on_a_clean_transaction() {
        let tx = tx_with(vec!["Program Vote111 success".to_string()], None);
        assert!(tx.is_success());
    ...
    #[test]
    #[should_panic(expected = "expected custom program error 0x7")]
    fn assert_error_code_rejects_the_wrong_code() {
        tx_with(
            vec!["custom program error: 0x9".to_string()],
            Some("Custom(9)".to_string()),
        )
        .assert_error_code(7);
    }
