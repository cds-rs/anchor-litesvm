...
/// The run environment a test declares before the first send: a value the
/// backend reads, not a sequence of imperative knobs. A new environment
/// parameter is a new field here, not a new trait method.
// ANCHOR: environment-config
#[derive(Debug, Clone)]
pub struct EnvironmentConfig {
    /// The transaction compute-unit ceiling every engine is pinned to. An
    /// engine's native default is its own (litesvm's solana graph defaults to
    /// the old per-instruction 200,000; agave 4.0 reports the 1,400,000
    /// per-transaction max), and that difference renders as the `M` in
    /// `consumed N of M`, so a cross-engine comparison must declare it equal.
    pub compute_unit_limit: u32,
    // future knobs: feature_set, sysvars, clock, preloaded programs ...
}

impl Default for EnvironmentConfig {
    fn default() -> Self {
        // The one home for the normalized ceiling: agave 4.0's per-transaction max.
        Self { compute_unit_limit: 1_400_000 }
    }
}
// ANCHOR_END: environment-config

/// The SVM a test executes against. One trait; one adapter crate per engine.
// ANCHOR: trait-core
/// The fee payer's pubkey from a signer set: `signers[0]` by convention, or
/// `None` for a signer-less build. The one home for the payer-selection rule,
/// so adapters building the legacy `Message` for the model agree on it.
pub fn payer_pubkey(signers: &[&Keypair]) -> Option<Pubkey> {
    use solana_signer::Signer;
    signers.first().map(|kp| kp.pubkey())
}
...
pub trait TestSVM {
    ...
    /// Default: the owner of the account's committed state. Override only for an
    /// engine with a cheaper native owner query than a full `get_account`.
    ...
    /// Apply a run environment before the first send. The default is a no-op,
    /// for an endpoint that runs on an environment it does not own (a stock
    /// RPC cluster). An engine that owns its environment destructures the
    /// config and honors each field through its own native knob; the struct is
    /// deliberately not `#[non_exhaustive]`, so a new field is a compile error
    /// in every adapter until each decides how to honor or ignore it.
    fn configure(&mut self, _config: &EnvironmentConfig) {}
