...
//! plain and syn-visible; this tool is the "build step" that reads it. It walks
...
fn main() {
    ...
    let idl = match build_idl(&crate_root, &program_id, name) {
        Ok(idl) => idl,
        Err(e) => {
            eprintln!("error: {e}");
            std::process::exit(1);
...
fn build_idl(crate_root: &Path, program_id: &str, name: Option<String>) -> Result<Value, String> {
