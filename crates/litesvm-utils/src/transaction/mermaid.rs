...
use {
    ...
    crate::transaction::model::{CpiModel, FrameLog, Outcome, ResolvedFrame},
    std::fmt::Write,
};
...
mod tests {
    ...
    fn render_with_logs(
        ...
        let mut collector =
            crate::transaction::renderer::LegendCollector::new(aliases, &empty_events);
        let model = crate::transaction::model::build(
            None,
            logs,
            inner_instructions,
            ...
            0,
            0,
            // Logs-only path (no TransactionResult, so no registered names):
            // built-in decoders still resolve System / SPL / ATA frames.
...
";
    ...
    fn no_signers_falls_back_to_user_placeholder() {
        ...
        let aliases =
            crate::Aliases::with_well_known().with(Pubkey::from_str(amm_id).unwrap(), "amm");
        let out = render_with(&logs, &Vec::new(), &aliases, &empty_signers(), Mode::Plain);
        assert!(
    ...
    fn undecoded_instruction_renders_safe_label_not_bare_question_mark() {
        ...
        let aliases =
            crate::Aliases::with_well_known().with(Pubkey::from_str(prog).unwrap(), "prog");
        let out = render_with(&logs, &Vec::new(), &aliases, &empty_signers(), Mode::Plain);
        assert!(
