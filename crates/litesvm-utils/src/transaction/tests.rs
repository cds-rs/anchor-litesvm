...
use solana_sdk::system_instruction as system_instruction;
...
// Structured-tree rendering tests, restored after the local
// `cpi_tree` port (litesvm 0.12's parser inlined as `crate::cpi_tree`)
// brought back the annotated CPI tree on this branch.
...
fn print_logs_structured_produces_annotated_output_for_real_transaction() {
    ...
    assert!(
        out.contains("└── System [1]"),
        "expected System alias substitution; got:\n{out}"
    );
}
...
fn logs_structured_string_header_falls_back_when_decoder_absent() {
    ...
    // recognized tags are 0..=12; we use 0xFF_FF_FF_FF as a tag the
    // table doesn't know.
    ...
    assert!(
        out.lines().any(|l| l.starts_with("── System ─") && !l.starts_with("── System::")),
...
fn logs_structured_string_without_attached_aliases_uses_default() {
    ...
    assert!(
        out.contains("└── System [1]"),
        "well-known System alias should still resolve via default; got:\n{out}"
    );
    assert!(
        !out.contains("Legend"),
