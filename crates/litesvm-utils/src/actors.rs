...
//! until you want to *commit* test output (structured logs, [`Report`]s): the
...
pub fn deterministic_keypair(domain: &str, role: &str) -> Keypair {
    // LTS / solana 2.x: no `Keypair::new_from_array`. Derive from the
    // 32-byte seed via `keypair_from_seed`, which uses the bytes as the
    // ed25519 secret seed. Deterministic for a fixed input, which is all
    // this needs; the resulting addresses are this branch's own and need
    // not match main's.
    keypair_from_seed(&seed_bytes(&format!("{domain}:{role}")))
        .expect("a 32-byte seed always yields a valid ed25519 keypair")
}
...
impl ActorRegistry {
    ...
    pub fn new(domain: impl Into<String>) -> Self {
        Self { domain: domain.into(), used: HashSet::new() }
    }
