...
fn to_frames(frames: Vec<litesvm::cpi_tree::CpiFrame>) -> Vec<testsvm::frame::Frame> {
    frames
        ...
        .map(|f| testsvm::frame::Frame {
            ...
            // Operands are decoded centrally from the trace in `assemble`; the
            // log parse carries none.
            operands: Vec::new(),
            logs: f
                .logs
                .into_iter()
...
impl TestSVM for LiteSvmBackend {
    ...
    fn configure(&mut self, config: &testsvm::EnvironmentConfig) {
        ...
        // litesvm exposes only a consuming builder, no post-hoc setter. Configure
        // before the first send, where swapping the still-empty SVM is clean:
        // `take` leaves a default placeholder while the real SVM (with its
        // builtins and state) is rebuilt with the new budget and put back. The
        // budget defaults to None (litesvm falls back to an internal default),
        // and ComputeBudget's `Default` is dev-only-gated, so seed a fresh one
        // with `new_with_defaults` when none is set. The (true, true) SIMD gates
        // assume the current/mainnet feature set, which the probe and typical
        // tests use; litesvm's `feature_set` is not publicly readable, so the
        // exact match cannot be derived here. A scenario that BOTH disables
        // those gates AND relies on the unset-budget default would get different
        // per-frame nesting limits from this seed; the CU ceiling we set is
        // unaffected.
        let mut budget = self
            .inner
            .get_compute_budget()
            .unwrap_or_else(|| ComputeBudget::new_with_defaults(true, true));
        budget.compute_unit_limit = u64::from(*compute_unit_limit);
        self.inner = std::mem::take(&mut self.inner).with_compute_budget(budget);
    }
